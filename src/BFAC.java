...
public class BFAC implements Runnable {
	...
	private void loopLevel(URL originalUrl, String cleanPath) {
		...
		for (i = 0; i <= runLevel; i++) {
			for (String file : levelsList.get(i)) {
				...
				// this.extender.stdout.println("Thread : "+dirPath + file + " ==>
				// "+Integer.toString(thList.size()));
	...
	private void processRespList() {
		// Unload Queue and print to doc. Doc is not thread safe, thats why we need a
		// queue
		while (!repList.isEmpty()) {
			...
			try {
				...
				doc.insertString(doc.getLength(),
						"(Response-Code: " + respCode + " | Content-Length: " + contLen + ")\n", null);
